...
export default {
  ...
  mounted() {
    if (this.redirecUrl && this.loginUrl) {
      ...
      this.handleAfterFbCallBack()
    }
  },
  methods: {
    async handleAfterFbCallBack() {
      try {
        ...
        if (code && state) {
          ...
          await this.insertChannelAccount(code, auth)
        } else if (error) {
    ...
    async insertChannelAccount(code, auth) {
      const redirectUrl = localStorage.getItem(this.redirectUrlKey)
      try {
        const res = await this.$api.socialChannel.insertChannelAccount(
          code,
          auth,
          'fb'
        )
        const data = await this.$api.socialChannel.insertChannel(
          res.data.id,
          auth,
          'fb'
        )
