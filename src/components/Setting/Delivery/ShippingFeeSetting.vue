    ...
    <div class="drawer-shipping-fee-setup__content">
      ...
      >
        <div class="drawer-shipping-fee-setup-content-body__content">
          ...
          >
            <div class="drawer-shipping-fee-setup-content-body__content--title">
              {{ $t('select-requirement') }}
            </div>
            <div>
              <a-form-model-item
                ref="shipping_fee_method"
                class="form-field"
                prop="shipping_fee_method"
                :colon="false"
              >
                <a-radio-group v-model="form.shipping_fee_method">
                  <template v-for="(option, index) in optionsRequirement">
                    <a-radio
                      :key="index"
                      :style="radioStyle"
                      :value="option.value"
            ...
            <div
              v-if="form.shipping_fee_method === shippingFeeMethod.dynamic"
              class="integrated-delivery-alert-info"
            >
              <a-alert
                :message="$t('shipping-fee-method-dynamic-alert')"
                type="info"
                show-icon
              />
            </div>
            <div
              v-else-if="form.shipping_fee_method === shippingFeeMethod.fixed"
            >
              <a-form-model-item
                :label="$t('choose')"
                class="form-field"
      ...
      if (this.form.shipping_fee_method === this.shippingFeeMethod.dynamic) {
        ...
        this.form.shipping_fee_config.condition_value.forEach((item) => {
          ...
          if (item.id > 0) {
            ...
            (item.shipping_fee === 0 ||
              item.shipping_fee === '' ||
              typeof parseInt(item.shipping_fee) !== 'number' ||
              item.shipping_fee < 0)
          ) {
            error = true
          }
