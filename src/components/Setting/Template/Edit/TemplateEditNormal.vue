...
export default {
  ...
  watch: {
    form: {
      handler(form) {
        this.canSubmit =
          !isEqual(form.name, this.templateEdit.name) ||
          !isEqual(form.response, this.templateEdit.response) ||
    ...
    async handleSubmit() {
      try {
        ...
        const payload = {
          name: this.form.name,
          type: 1,
          conversation_type: this.templateEdit.conversation_type
            ? this.templateEdit.conversation_type
            : 0,
