          ...
          >
            ...
            <template v-if="form.autoResponse">
              <a-form-model-item style="margin-bottom: 0">
                <div class="flex items-center">
                  <a-checkbox ref="isSetActiveDate" v-model="isSetActiveDate">
        ...
        this.canSubmit =
          ...
          (form.selectedDate[1] &&
            this.$moment(form.selectedDate[1], 'HH:mm DD/MM/YYYY')
              .utc()
              .format('YYYY-MM-DD HH:mm') !==
              this.$moment(this.templateEdit.end_date).format(
                'YYYY-MM-DD HH:mm'
              )) ||
          (form.autoResponse && !this.templateEdit.auto_response) ||
          (!form.autoResponse && this.templateEdit.auto_response)
  ...
  created() {
    if (this.templateEdit) {
      ...
      this.form = {
        ...
        autoResponse: this.templateEdit.auto_response === 1,
        isKeywords: [...this.templateEdit.is_keyword],
        hasKeywords: [...this.templateEdit.has_keyword],
        templateType: this.templateEdit.is_keyword?.length
          ? this.enumTemplateType.IS_KEYWORD
          : this.enumTemplateType.HAS_KEYWORD,
        selectedDate: [
          this.templateEdit.start_date.length &&
            convertUTCToLocal(this.templateEdit.start_date, 'HH:mm DD/MM/YYYY'),
          this.templateEdit.end_date.length &&
            convertUTCToLocal(this.templateEdit.end_date, 'HH:mm DD/MM/YYYY'),
        ],
      }
    }
  },
      ...
      try {
        ...
        const payload = {
          ...
          ...(form.templateType === this.enumTemplateType.IS_KEYWORD
            ? { is_keyword: [...form.isKeywords], has_keyword: [] }
            : { has_keyword: [...form.hasKeywords], is_keyword: [] }),
          ...(form.autoResponse && { start_date: startDate }),
          ...(form.autoResponse && { end_date: endDate }),
        }
