  ...
  methods: {
    getTitleType(key) {
      const typeResponse = [
        ...
        {
          key: 2,
          text: this.$t('auto-suggestion'),
        },
        {
          key: 3,
          text: this.$t('auto-response'),
        },
      ]
      const findType = typeResponse.find((item) => item.key === key)
    ...
    handleOpenEditTemplate(template) {
      const typeTemplate = [
        { type: 1, component: 'edit-normal' },
        { type: 2, component: 'edit-auto' },
        { type: 3, component: 'edit-auto' },
      ]
      const findTypeTemplate = typeTemplate.find(
        (item) => item.type === template.type
      )
      if (findTypeTemplate !== -1) {
        this.isShowEditTemplate = true
        this.isTypeComponent = findTypeTemplate.component
