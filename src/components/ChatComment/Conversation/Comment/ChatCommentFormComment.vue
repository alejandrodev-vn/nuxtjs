<template>
  <div class="conversation-bottom">
    <transition name="float-up" mode="out-in">
      <embedded-message
        v-if="activeComment"
        :active-message="activeComment"
        @handleCancelReplyMessage="handleCancelReplyComment"
      />
    </transition>
    <a-button
      type="secondary"
    ...
    <div class="conversation-bottom__form">
      <div v-if="fileList.length && listPreview.length" class="preview-list">
        ...
        >
          ...
          />
          <img v-if="file.type === 'image'" :src="file.blob" :alt="file.name" />
          <video v-else-if="file.type === 'video'" :src="file.blob"></video>
        </div>
      </div>
      <div class="form-enter" :class="{ disable: isLock || !activeCommentId }">
        <a-textarea
          v-model="content"
          size="large"
          class="custom-textarea"
          :auto-size="{ minRows: 1, maxRows: 6 }"
          :disabled="isLock || !activeCommentId"
          @pressEnter="handlePressEnter"
        >
        </a-textarea>
        <input
          v-if="!disableFileInput"
          id="attachmentFile"
          ref="attachmentFile"
          :disabled="isLock || !activeCommentId || disableFileInput"
          :style="{ display: 'none' }"
          type="file"
        ...
        />
        <label v-if="!disableFileInput" for="attachmentFile">
          <icon-attach
            class="icon-attach"
            :class="{ disable: isLock || !activeCommentId }"
          />
        </label>
      </div>
    </div>
    <a-button
      ...
      type="primary"
      :class="{ disable: isLock || !activeCommentId }"
      @click="handleSubmitReplyComment"
    >
    ...
    <app-modal
      ...
      :ok-text="$t('confirm')"
      @confirm="handleConfirmWarningToReplyComment"
    >
...
export default {
  ...
  watch: {
    ...
    infoPage() {
      ...
      if (find) {
        this.acceptFile = find.acceptFile
        this.disableFileInput = find.disableFileInput
        return
      }
  ...
  methods: {
    ...
    checkKeyword() {
      const matched = this.unsafeKeywords.filter(
        (item) =>
          (!item.conversation_type || item.conversation_type === 2) &&
          this.content
            .toLowerCase()
    ...
    handleConfirmWarningToReplyComment() {
      const listIdUnsafeKeywords = this.containUnsafeKeywords.map(
        (keyword) => keyword.id
      )
      this.$emit('handleSubmitReplyComment', {
    ...
    handleUseResponseTemplate(value) {
      if (!value.length || !this.activeCommentId) return
      this.content = value
    },
