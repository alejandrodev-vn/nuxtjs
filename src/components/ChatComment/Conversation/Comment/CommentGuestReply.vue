<template>
  <div
    ref="wrapComment"
    class="wrap-comment"
    :class="{ 'show-background': isShowBackgroundWrapChildren || isLast }"
  >
    <div class="comment-guest" :class="{ 'last-comment': isLast }">
      ...
      <div class="comment-guest__right">
        <div class="top">
        ...
        <div class="action" :class="{ isCollapsed }">
          ...
          <span
            v-if="commentInline.is_deleted || parentDeleted.is_deleted"
            class="action__deleted italic mr-4"
          >
            {{ $t('deleted') }}
          </span>
          ...
          <!-- <div v-else class="comment-actions">
            ...
            >
              ...
              <div
                class="comment-actions__icon comment-actions__icon--tag"
                :class="{
                  active: isTagged,
                }"
            ...
            >
              ...
              <div
                class="comment-actions__icon comment-actions__icon--skip"
                :class="{
                  active: isSkipped,
                  '!cursor-default': isSkipped,
                }"
                @click="handleClickSkipComment"
              >
            ...
            >
              <template slot="title">
                <span>
                  {{ isResponded ? $t('responded') : $t('response') }}
                </span>
              </template>
              <div
                class="comment-actions__icon comment-actions__icon--reply"
                :class="{
                  active:
                    activeCommentId === commentInline.conversation_id ||
                    isResponded,
                  '!cursor-default': isResponded,
                }"
                @click="handleClickReplyComment"
              >
            ...
            >
              ...
              <div
                class="comment-actions__icon comment-actions__icon--escalate"
              >
            ...
            >
              <template slot="title">
                <span>
                  {{ commentInline.is_hidden ? $t('unhide') : $t('hide') }}
                </span>
              </template>
              <div
                class="comment-actions__icon comment-actions__icon--hide"
                :class="{
                  active: commentInline.is_hidden,
                }"
                @click="handleClickHideComment(commentInline)"
              >
    ...
    <div class="comment-guest__replies">
      <template
        v-if="infoPage && commentInline.replies && commentInline.replies.data"
      >
        <template v-for="(response, index) in commentInline.replies.data">
          <comment-response
            v-if="
              response.content.length ||
              (response.attachments && response.attachments.length)
            "
            :key="index"
            :response="response"
            :info-page="infoPage"
            ...
            :style="{ paddingLeft: '5.8rem', marginLeft: ' 2.4rem' }"
...
export default {
  ...
  methods: {
    ...
    handleUpdateComment(newData) {
      ...
      this.rerenderVerticalLine()
      this.$emit('rerenderVerticalLine')
    },
